...
def get_s3_instance():
    # init s3 in us-east-2 for the sync-dev-server bucket
...
def get_redis_instance():
    pool = redis.ConnectionPool(
        host=cfg.REDIS_HOST, password=cfg.REDIS_PASSWORD, port=cfg.REDIS_PORT, db=0
    )
...
def download_s3_file(file_path, local_path, bucket_name=cfg.S3_NAME):
    try:
        ...
        finish = time.time()
        logging.info(
            f"Avatar downloaded successfully to {local_path} in {finish - start}s"
        )
        return local_path
...
def upload_timestamps(meeting_id, start, end):
    try:
        ...
        data = {"model-emotion-detection": [int(start * 1000), int(end * 1000)]}
