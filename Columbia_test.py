...
def visualization(tracks, scores, args):
    ...
    for fidx, fname in tqdm.tqdm(enumerate(flist), total=len(flist)):
        image = cv2.imread(fname)
        timestamp = fidx / fps  # Calculate timestamp based on FPS
        for face in faces[fidx]:
            clr = colorDict[int((face['score'] >= 0))]
            txt = round(face['score'], 1)
            cv2.rectangle(image, (int(face['x'] - face['s']), int(face['y'] - face['s'])), (int(face['x'] + face['s']), int(face['y'] + face['s'])), (0, clr, 255 - clr), 10)
            cv2.putText(image, '%s' % (txt), (int(face['x'] - face['s']), int(face['y'] - face['s'])), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, clr, 255 - clr), 5)
            # Print the active speaker details
            if face['score'] >= 0:
                print(f"Active Speaker: speaker_{face['track']}, Timestamp: {timestamp:.2f}, BBox: ({int(face['x'] - face['s'])}, {int(face['y'] - face['s'])}, {int(face['x'] + face['s'])}, {int(face['y'] + face['s'])})")
        vOut.write(image)
    vOut.release()
